...
def parse_generated_samples(file_path, num_graphs=3, weighted=False):
    """Parse generated_samples.txt and extract graphs (binary or weighted)"""
    graphs = []
    ...
    for entry in entries[:num_graphs]:
        ...
        for i, e_line in enumerate(e_lines):
            if not e_line.strip():
                break
            row = list(map(float, e_line.split()))
            for j, val in enumerate(row):
                if i < j:
                    if weighted:
                        if val != 0:
                            edges.append((i, j, int(val)))   # weighted edge
                    else:
                        if val == 1:
                            edges.append((i, j))        # binary edge
...
def plot_graphs(graphs, output_dir=None):
    ...
    for idx, graph_data in enumerate(graphs):
        ...
        # Detect if weighted
        is_weighted = all(len(e) == 3 for e in graph_data['edges'])

        if is_weighted:
            G.add_weighted_edges_from(graph_data['edges'])
        else:
        ...
        # Draw edge labels only if weighted
        if is_weighted:
        ...
        ax.set_title(
            f'Generated Graph {idx+1}\n'
            f'({graph_data["n_nodes"]} nodes, {len(graph_data["edges"])} edges)',
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Plot generated graphs from generated_samples.txt (binary or weighted)'
    )
    ...
    parser.add_argument('-w', '--weighted', action='store_true',
                        help='Treat adjacency matrix as weighted and draw edge labels')
    ...
    all_graphs = parse_generated_samples(
        args.file,
        num_graphs=10000,
